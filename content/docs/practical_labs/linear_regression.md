...
weight: 10
---
# Linear Regression using Gradient Descent: Application to Matrix Factorization
...
In the context of spectral unmixing, this means factorizing the observed data matrix $ \mathbf{Y} $ into an endmember matrix $ \mathbf{S} $ and an abundance matrix $ \mathbf{A} $. This can be formulated as an optimization problem where the goal is to minimize the difference between the observed data and the reconstructed data.

**3.** *Propose an objective function for this optimization problem.*

**4.** *Can you find a closed form solution for $\mathbf{A}$ and $\mathbf{S}$.*

**5.** *Is the objective function convex ?*
...
The Block-coordinate descent is an optimization algorithm that updates a subset of variables at each iteration, which is particularly useful for problems with separable structure. In the context of spectral unmixing, this method can be used to solve the optimization problem for $ \mathbf{A} $ and $ \mathbf{S} $ simultaneously.

The intuition behind block-coordinate descent is to break down a complex optimization problem into simpler subproblems that can be solved more efficiently. The algorithm iterates by fixing one set of variables and optimizing the other, cycling through all blocks until convergence.
...
**6.** *Propose and implement a Block-coordinate descent scheme to solve for $ \mathbf{A} $ and $ \mathbf{S} $*
...
**7.** *Can you comment on the results obtained ?*
...
The result of the least squares is non-unique because the dimension of the kernel of the gradient equation is greater than 1. This non-uniqueness arises because multiple solutions can satisfy the same gradient condition, leading to different possible solutions for the same problem.

Thus, regularization techniques are used to stabilize the solution and reduce the impact of noise by restricting the solution space. They can be implemented as constraints in the optimization problem or relaxed to allow some flexibility. Ridge regression is one such technique that introduces a regularization term to the objective function:

$$ \min_{\mathbf{A}, \mathbf{S}} \Vert\mathbf{A} \mathbf{S} - \mathbf{Y}\Vert_F^2 + \lambda \Vert\mathbf{A}\Vert_F^2 $$
...
### Today: Ridge Regression

Ridge regression is a powerful regularization technique used to address ill-posed inverse problems. By introducing a penalty term to the least squares objective, ridge regression helps stabilize the solution and reduce overfitting.

The equation for ridge regression is given by:

$$ \min_{\mathbf{A}} \Vert\mathbf{A} \mathbf{S} - \mathbf{Y}\Vert_F^2 + \lambda \Vert\mathbf{A}\Vert_F^2 $$

This formulation is closely related to Tikhonov regularization, which is used to solve ill-posed problems by adding a regularization term that penalizes the norm of the solution. The rationale behind ridge regression is to introduce a bias that reduces the variance of the estimated parameters, thereby improving the generalization of the model.
